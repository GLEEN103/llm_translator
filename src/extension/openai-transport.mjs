...
export const OPENAI_PORT = 'openai-private-transport';
const FILE = 'openai-offscreen.html';
const errors = new Set(['PROVIDER_RESPONSE_TOO_LARGE', 'OPENAI_REQUEST_TIMEOUT', 'OPENAI_NETWORK', 'INVALID_REQUEST', 'OPENAI_WORKER_START_FAILED']);

// Background owns authorization and accounting. Only the actual offscreen document gets this port.
export function createOpenAITransport(chrome) {
  ...
  const fail = () => { for (const entry of [...pending.values()]) entry.finish(new AppError('OPENAI_TRANSPORT_DISCONNECTED')); };
  chrome.runtime.onConnect.addListener(candidate => {
    if (candidate.name !== OPENAI_PORT) return;
    void (async () => {
      ...
      candidate.onMessage.addListener(message => {
        ...
        if (!entry) return;
        if (message.type === 'error') { entry.finish(new AppError(errors.has(message.code) ? message.code : 'OPENAI_NETWORK')); return; }
        if (message.type !== 'result' || !Number.isInteger(message.status) || message.status < 200 || message.status > 599 ||
          ...
          entry.finish(new AppError('OPENAI_RESPONSE_FORMAT')); return;
        }
        try {
        ...
        } catch { entry.finish(new AppError('OPENAI_RESPONSE_FORMAT')); }
      });
      attempt.resolve();
  ...
  const ensure = () => lock(async () => {
    ...
    const ready = new Promise((resolve, reject) => { connecting = { resolve, reject, url: chrome.runtime.getURL(url) }; timer = setTimeout(() => reject(new AppError('OPENAI_TRANSPORT_START_TIMEOUT')), 8000); });
    try {
      await Promise.all([ready, chrome.offscreen.createDocument({ url, reasons: ['WORKERS'],
        justification: 'Run bounded OpenAI translation network requests in a dedicated worker while the service worker handles authorization and accounting.' })]);
    } catch (error) {
      throw error instanceof AppError ? error : new AppError('OPENAI_TRANSPORT_START_FAILED');
  ...
  return async (url, options) => {
    ...
    try {
      ...
      if (!port) throw new AppError('OPENAI_TRANSPORT_DISCONNECTED');
      return await new Promise((resolve, reject) => {
        ...
        timer = setTimeout(() => finish(new AppError('OPENAI_REQUEST_TIMEOUT')), OPENAI_TIMEOUT_MS);
        try { target.postMessage({ type: 'fetch', id, url, authorization: options.headers?.Authorization, body: options.body }); }
        catch { finish(new AppError('OPENAI_TRANSPORT_DISCONNECTED')); }
      });
